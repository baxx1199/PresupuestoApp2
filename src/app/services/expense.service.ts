...
export class IExpense{
    ...
    addNewExpense(descriptionNew:string, valueNew:number, createdAt:Date){
    this.expense.push( new Expense(descriptionNew,valueNew,createdAt));
        let expensesTemp: Expense[]=[];

        if(localStorage.getItem("Expenses")===null){
            expensesTemp.push( new Expense(descriptionNew,valueNew,createdAt));
