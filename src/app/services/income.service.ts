...
export class IIncome{
    ...
    addNewIncome(descriptionNew:string, valueNew:number, createdAt:Date){
        this.incomes.push( new Income(descriptionNew,valueNew,createdAt ));
        
        let incomesTemp: Income[]=[];

        if(localStorage.getItem("incomes")===null){
            incomesTemp.push( new Income(descriptionNew,valueNew,createdAt));
